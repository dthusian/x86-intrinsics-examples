...
void vmath256_add_i8(int8_t* a, const int8_t* b, size_t sz) {
  ...
  for (i = 0; i < sz; i += 32) { // m256 / int8 = 32
    ...
    __m256i mma = _mm256_loadu_si256(a + i); // All intrinsics are now _mm256_* prefixed.
    __m256i mmb = _mm256_loadu_si256(b + i); // Your SSE instructions still work, like loadu_*
    __m256i mmc = _mm256_add_epi8(mma, mmb); // As you can probably infer, this adds the two registers, interpreting contents as int8.
    _mm256_storeu_si256(a + i, mmc);
  }
...
void vmath256_add_i16(int16_t* a, const int16_t* b, size_t sz) {
  ...
  for (i = 0; i < sz; i += 16) { // m256 / int16 = 16
    __m256i mma = _mm256_loadu_si256(a + i);
    __m256i mmb = _mm256_loadu_si256(b + i);
    __m256i mmc = _mm256_add_epi16(mma, mmb);
    _mm256_storeu_si256(a + i, mmc); // Unlike SSE, there are seperate integer storage instructions for each integer type.
  }
